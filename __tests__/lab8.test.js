describe('Basic user flow for Website', () => {
  ...
  it('Checking number of items in cart on screen', async () => {
    ...
    for (let index = 1; index < prodItems.length; index++) {
      // Grab the .shadowRoot property of <product-items> to grab all of the button stored inside
      shadowRoot = await prodItems[index].getProperty('shadowRoot');
      //console.log(`Checking product item ${index + 1}/${prodItems.length}`);
      const button = await shadowRoot.$('button')
      await button.click()
    }
    ...
    const allArePopulated = await page.$('#cart-count')
    const count = await allArePopulated.getProperty('innerText');
